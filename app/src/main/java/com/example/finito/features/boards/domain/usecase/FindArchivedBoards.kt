...
class FindArchivedBoards(
    ...
    operator fun invoke(
        boardOrder: SortingOption.Common = SortingOption.Common.Default,
        searchQuery: String? = null,
        vararg labelIds: Int,
    ): Result.Success<Flow<List<BoardWithLabelsAndTasks>>> {
        return Result.Success(
            data = repository.findArchivedBoards().map { boards ->
    ...
    private fun sortBoards(
        ...
        SortingOption.Common.Custom -> boards.sortedBy { it.board.archivedAt }
    }
}